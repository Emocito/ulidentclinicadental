...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const injectMobileStickyCta = () => {
    ...
    stickyCta.className = 'fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white/95 backdrop-blur-md border-t border-slate-200/80 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] flex justify-between items-center px-4 py-3 gap-3 pb-safe';
    
    stickyCta.innerHTML = `
      <!-- Llamar -->
      <a href="tel:+34910248447" class="w-11 h-11 rounded-full bg-brand-bluelight hover:bg-blue-100 text-brand-blue flex items-center justify-center transition-colors border border-brand-blue/10 flex-shrink-0" aria-label="Llamar por teléfono">
        <i data-lucide="phone" class="w-5 h-5 text-brand-blue"></i>
      </a>

      <!-- WhatsApp -->
      <a href="https://wa.me/34679926552" target="_blank" rel="noopener noreferrer" class="w-11 h-11 rounded-full bg-[#25D366] hover:bg-[#20BA56] text-white flex items-center justify-center transition-all duration-300 hover:scale-105 shadow-sm flex-shrink-0" aria-label="Contactar por WhatsApp">
        <svg viewBox="0 0 24 24" class="w-5.5 h-5.5 fill-current">
      ...
      <!-- Pedir Cita -->
      <a href="index.html#reserva" class="flex-grow h-11 bg-brand-blue hover:bg-brand-bluedark text-white rounded-full flex items-center justify-center font-heading font-extrabold text-xs uppercase tracking-wider transition-all duration-300 shadow-md hover:shadow-lg">
